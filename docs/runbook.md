...
Hướng dẫn vận hành hệ thống AI Agent Social Automation.
...
### Add New Topic Idea
```sql
docker exec -it postgres psql -U postgres -d social_automation -c "
...
gunzip -c ~/backups/social-automation/postgres_YYYYMMDD_HHMMSS.sql.gz | \
  docker exec -i postgres psql -U postgres -d social_automation
...
# Database size
docker exec postgres psql -U postgres -d social_automation -c "
SELECT pg_size_pretty(pg_database_size('social_automation'));
"
...
alias sa-db="docker exec -it postgres psql -U postgres -d social_automation"
alias sa-pending="docker exec postgres psql -U postgres -d social_automation -c \"SELECT id, title, pillar FROM content_queue WHERE status = 'pending_review' ORDER BY created_at DESC;\""
```
